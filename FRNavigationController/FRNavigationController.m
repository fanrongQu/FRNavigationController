...
//FRSrcName(file) 为通过copy文件夹方式获取图片路径的宏
//FRFrameworkSrcName(file) 为通过cocoapods下载安装获取图片路径的宏
#define FRSrcName(file) [@"FRNavigationController.bundle" stringByAppendingPathComponent:file]
#define FRFrameworkSrcName(file) [@"Frameworks/FRNavigationController.framework/ZLPhotoBrowser.bundle" stringByAppendingPathComponent:file]
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (self.viewControllers.count > 0) {//这时push进来的控制器不是第一个控制器
        ...
        UIImage *backImage = [UIImage imageNamed:FRSrcName(@"navigationbar_back.png")]?:[UIImage imageNamed:FRFrameworkSrcName(@"navigationbar_back.png")];
        
        if ([[UIBarButtonItem appearance].tintColor isEqual:[UIColor whiteColor]]) {
            backImage = [UIImage imageNamed:FRSrcName(@"navigationbar_white.png")]?:[UIImage imageNamed:FRFrameworkSrcName(@"navigationbar_white.png")];
        }
